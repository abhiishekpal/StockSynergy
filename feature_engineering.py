...
class FeatureEngineering:
  ...
  def get_price_diff(self, data):
    ...
    joined_data = pd.merge(data1, data2, left_on='future_date', right_on='date', how='inner')[["date_x", "price", "future_date", "future_price"]]
    joined_data.columns = ["current_date", "price", "future_date", "future_price"]

    joined_data["price_diff"] = (joined_data["future_price"]-joined_data["price"])/joined_data["price"]
  ...
  def _get_uncertainity(self, x, window):
    ...
    if x[f"cutoff_point_{window}_low"]<x["price_diff"]< x[f"cutoff_point_{window}_high"]:
      return 1
    return 0
  ...
  def add_rolling_average(self, data):
    ...
    for window in self.windows:
      data[f"mean_diff_{window}"] = data['price_diff'].rolling(window=window).mean()
      data[f"std_diff_{window}"] = data['price_diff'].rolling(window=window).mean()
    for window in self.windows:
      data[f"cutoff_point_{window}_high"] = data.apply(lambda x: x[f"mean_diff_{window}"] + cutoff_point(x[f"mean_diff_{window}"], x[f"std_diff_{window}"], 80), axis=1)
      data[f"cutoff_point_{window}_low"] = data.apply(lambda x: x[f"mean_diff_{window}"] - cutoff_point(x[f"mean_diff_{window}"], x[f"std_diff_{window}"], 80), axis=1)
  ...
  def get_feature(self, data):
    ...
    for window in self.windows:
      data[f"in_{window}"] = data.apply(lambda x: self._get_uncertainity(x, window), axis=1)
